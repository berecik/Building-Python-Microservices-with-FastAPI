...
@router.post("/bid/add")
def add_bid(req: BidsReq, current_user: Login = Depends(get_current_user), sess:Session = Depends(sess_db)): 
...
def update_bid(id:int, req: BidsReq, current_user: Login = Depends(get_current_user), sess:Session = Depends(sess_db)): 
    bid_dict = req.dict(exclude_unset=True)
    repo:BidsRepository = BidsRepository(sess)
...
def delete_bid(id:int, current_user: Login = Depends(get_current_user), sess:Session = Depends(sess_db)): 
    repo:BidsRepository = BidsRepository(sess)
...
@router.get("/bid/all")
def list_all_bid(current_user: Login = Depends(get_current_user), sess:Session = Depends(sess_db)): 
    repo:BidsRepository = BidsRepository(sess)
    return repo.get_all_bids()  

@router.post("/bid/{id}")
def get_bid(id:int, current_user: Login = Depends(get_current_user), sess:Session = Depends(sess_db)): 
    repo:BidsRepository = BidsRepository(sess)
